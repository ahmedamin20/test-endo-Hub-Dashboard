...
"use client";
import { ServicesResponse } from "@/api/interfaces/Services";
...
import { getServices, serviceDelete } from "@/api/queries/services.query";
...
export default function Home() {
    const headers = ["id", "name", 'image', 'price', 'description',];
    ...
    const { data, refetch, isLoading } = useQuery<ServicesResponse>({
        queryKey: [API_ENDPOINTS.SERVICES, page, per_page, handle],
        queryFn: () => getServices(per_page, page, handle),
    });

    const deleteAd = useMutation({
        mutationFn: () => serviceDelete(deleteId!),
        onSuccess: () => {
    ...
    return (
        <>
            <main className="flex flex-col gap-5 justify-center items-center px-[5px] py-10 w-full">
                <Head content={t("services.head")} />
                <div className="flex flex-row justify-between w-full">
                    <Search className="" onChange={handleSearch}></Search>
                    <MainLink href="/services/create" className="p-2 rounded-lg">
                        {t("add")}
                        <IoMdAdd />
                    </MainLink>
                </div>
