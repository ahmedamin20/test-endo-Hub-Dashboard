...
"use client";
import { CategoriesResponse, Category } from "@/api/interfaces/Categories";
import { AdsResponse, Member, TeamsResponse } from "@/api/interfaces/Teams";
import { adsDelete, getAds } from "@/api/queries/ads.query";
import { cateogryDelete, getCategories } from "@/api/queries/categories.query";
...
export default function Home() {
    ...
    const { data, refetch, isLoading } = useQuery<CategoriesResponse>({
        queryKey: [API_ENDPOINTS.CATEGORIES, page, per_page, handle],
        queryFn: () => getCategories(per_page, page, handle),
    });

    const deleteAd = useMutation({
        mutationFn: () => cateogryDelete(deleteId!),
        onSuccess: () => {
    ...
    return (
        <>
            <main className="flex flex-col gap-5 justify-center items-center px-[5px] py-10 w-full">
                <Head content={t("categories.head")} />
                <div className="flex flex-row justify-between w-full">
                    <Search className="" onChange={handleSearch}></Search>
                    <MainLink href="/categories/create" className="p-2 rounded-lg">
                        {t("add")}
                        <IoMdAdd />
                    </MainLink>
                </div>
