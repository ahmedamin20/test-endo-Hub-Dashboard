...
export default function Home() {
    const headers = ["id", "title", 'image', 'description', 'discount',];
    ...
    const { data, refetch, isLoading } = useQuery<AdsResponse>({
        queryKey: [API_ENDPOINTS.ADS, page, per_page, handle],
        queryFn: () => getAds(per_page, page, handle),
    });

    const deleteAd = useMutation({
        mutationFn: () => adsDelete(deleteId!),
        onSuccess: () => {
    ...
    return (
        <>
            <main className="flex flex-col gap-5 justify-center items-center px-[5px] py-10 w-full">
                <Head content={t("ads.head")} />
                <div className="flex flex-row justify-between w-full">
                    <Search className="" onChange={handleSearch}></Search>
                    <MainLink href="/ads/create" className="p-2 rounded-lg">
                        {t("add")}
                        <IoMdAdd />
                    </MainLink>
                </div>
