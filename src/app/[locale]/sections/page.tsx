...
import { getSections, getSectionsTable, sectionDelete } from "@/api/queries/sections.queries";
...
export default function Home() {
    ...
    const [sections, setSections] = useState<Member[]>();

    const { data, refetch, isLoading } = useQuery<TeamsResponse>({
        queryKey: [API_ENDPOINTS.SECTIONS, page, per_page, handle],
        queryFn: () => getSectionsTable(per_page, page, handle),
    });

    const deleteSection = useMutation({
        mutationFn: () => sectionDelete(deleteId!),
        onSuccess: () => {
    ...
    return (
        <>
            <main className="flex flex-col gap-5 justify-center items-center px-[5px] py-10 w-full">
                <Head content={t("sections.head")} />
                <div className="flex flex-row justify-between w-full">
                    <Search className="" onChange={handleSearch}></Search>
                    <MainLink href="/sections/create" className="p-2 rounded-lg">
                        {t("add")}
                        <IoMdAdd />
                    </MainLink>
                </div>
